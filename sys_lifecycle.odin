...
sys_lifecycle_collision_classify :: proc(
	...
	rel_speed_sq := rl.Vector2LengthSqr(e1.velocity - e2.velocity)
...
sys_lifecycle_resolve_merge :: proc(g: ^Game, e: ^Game_Event_Collision) {
	...
	vel_x := (e1.mass * e1.velocity.x + e2.mass * e2.velocity.x) / new_mass
	vel_y := (e1.mass * e1.velocity.y + e2.mass * e2.velocity.y) / new_mass
	...
	entity_add_velocity(g, id, new_vel)
	entity_add_life(g, id, {g.elapsed})
	entity_add_renderable(g, id, {})
