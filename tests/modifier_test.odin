...
@(test)
test_modifier_persistence_roundtrip :: proc(t: ^testing.T) {
	g1 := test_make_game()
	defer free(g1)

	game.modifier_add(g1, .Gravity_Boost)
	game.modifier_add(g1, .Energy_Magnet)
	g1.modifiers[0].timer.curr = 12.5

	buf := make([]u8, game.MAX_SAVE_SIZE)
	defer delete(buf)

	n, serialize_ok := game.persist_serialize(g1, buf)
	testing.expect(t, serialize_ok, "serialize should succeed")

	g2 := test_make_game()
	defer free(g2)
	deserialize_ok := game.persist_deserialize(g2, buf[:n])
	testing.expect(t, deserialize_ok, "deserialize should succeed")

	testing.expect(t, g2.modifiers_count == 2, "deserialized modifiers_count should be 2")
	testing.expect(t, g2.modifiers[0].kind == .Gravity_Boost)
	testing.expect(t, g2.modifiers[0].timer.curr == 12.5)
	testing.expect(t, g2.modifiers[1].kind == .Energy_Magnet)
	testing.expect(t, g2.modifiers[1].permanent)
}

@(test)
test_modifier_persistence_kind_guard :: proc(t: ^testing.T) {
	g1 := test_make_game()
	defer free(g1)
